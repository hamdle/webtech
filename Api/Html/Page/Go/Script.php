...
<script>
    function onStart(loadNext = true) {
        ...
        Instructions.hide();
        Workout.create();
        InputDisplay.init(document.getElementById('inputdisplay'), <?php echo \Api\Core\Database\Database::config("set_rest_default",  \Api\Core\Context::get('user')->fields["id"]) ?>);
        if (loadNext) {
            InputDisplay.next();
        }
        Timer.start();
        Countdown.start(<?php echo \Api\Core\Database\Database::config("set_rest_default", \Api\Core\Context::get('user')->fields["id"]) ?>);
    ...
    RoutineBuilder.init(document.getElementById('exercise__list'), JSON.parse('<?php echo json_encode(\Api\Core\Context::get('exerciseList')) ?>'));

    Timer.init(document.getElementById('timer'));
    var timer = document.getElementById('timer')
    timer.addEventListener('click', function () {
        Countdown.start(<?php echo \Api\Core\Database\Database::config("rep_rest_default", \Api\Core\Context::get('user')->fields["id"]) ?>);
    });

    Countdown.init(
        document.getElementById('countdown'),
        <?php echo \Api\Core\Database\Database::config("play_timer_sound", \Api\Core\Context::get('user')->fields["id"]) ?>
    );
